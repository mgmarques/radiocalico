...
Generate `SBOM.md` — a Software Bill of Materials listing every installed
Python and Node.js package with its version and known vulnerability status.

### What it produces

`SBOM.md` at the repo root with:
...
3. **Review the output** — open `SBOM.md` and check:
...
| --- | --- |
| **Security Auditor agent** | Reads SBOM.md as the dependency inventory for triage |
...
| **CI/CD** | `.github/workflows/update-sbom.yml` auto-updates SBOM.md on every PR |
...
- Do NOT hardcode versions in `SBOM.md` — always regenerate from the actual installed state
- Do NOT commit SBOM.md with fabricated results — the script must run and produce real output
